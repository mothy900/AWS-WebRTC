...
export class DemoMeetingApp implements AudioVideoObserver, DeviceChangeObserver, ContentShareObserver {
  ...
  totalBandwidth: string | null = null;
  ...
  initEventListeners(): void {
    ...
    // this.audioVideo.chooseVideoInputQuality(3840, 2160, 60, 5*1024*1024);//5Gbps

    // console.log(videoInputQuality.value);
    ...
    videoInputQuality.addEventListener('change', async (_ev: Event) => {
      ...
      switch (videoInputQuality.value) {
        ...
        case '360p':
          console.log('360p True');
          this.audioVideo.chooseVideoInputQuality(640, 360, 30, 600);//600Kbps
          break;

        case '540p':
          console.log('540p True');
          this.audioVideo.chooseVideoInputQuality(960, 540, 30, 140 * 1024);//140Mbps
          break;

        case '720p':
          console.log('720p True');
          this.audioVideo.chooseVideoInputQuality(1280, 720, 60, 1 * 1024 * 1024);//1Gbps
          break;

        case '1080p':
          console.log('1080p True');
          this.audioVideo.chooseVideoInputQuality(1920, 1080, 60, 3 * 1024 * 1024);//3Gbps
          break;

        case '2160p_18fps':
          console.log('2160p 18FPS True');
          this.audioVideo.chooseVideoInputQuality(3840, 2160, 18, 5 * 1024 * 1024);//5Gbps
          break;

        case '2160p_30fps':
          console.log('2160p 30FPS True');
          this.audioVideo.chooseVideoInputQuality(3840, 2160, 30, 5 * 1024 * 1024);//5Gbps
          break;

        case '2160p_60fps':
          console.log('2160p 60FPS True');
          this.audioVideo.chooseVideoInputQuality(3840, 2160, 60, 5 * 1024 * 1024);//5Gbps
          break;

        case '2160p_120fps':
          console.log('2160p 120FPS True');
          this.audioVideo.chooseVideoInputQuality(3840, 2160, 120, 5 * 1024 * 1024);//5Gbps
          break;

        case '2160p_144fps':
          console.log('2160p 144FPS True');
          this.audioVideo.chooseVideoInputQuality(3840, 2160, 144, 5 * 1024 * 1024);//5Gbps
          break;
      }
      try {
        await this.openVideoInputFromSelection(videoInput.value, true);
    ...
    test.addEventListener('click', _e => {
      new AsyncScheduler().start(() => {
        if (!this.isButtonOn('test')) {
          this.contentShareStart();
        } else {
          this.contentShareStop();
        }
    ...
    (buttonMeetingEnd.firstElementChild as SVGElement).classList.add('svg-default');
    ...
    //class 생성
    (buttonMeetingLeave.firstElementChild as SVGElement).classList.add('svg-default');
  ...
  metricsDidReceive(clientMetricReport: ClientMetricReport): void {
    ...
    if (typeof metricReport.availableSendBandwidth === 'number' && !isNaN(metricReport.availableSendBandwidth)) {
      (document.getElementById('video-uplink-bandwidth') as HTMLSpanElement).innerText =
        'Available Uplink Bandwidth: ' + String(Math.ceil(metricReport.availableSendBandwidth / 1000)) + ' Kbps'; //Math.ceil(metricReport.availableSendBandwidth / 1000) <- 소수점이 많이 나오면 사용

      // console.log('Available Uplink Bandwidth: ' +String(Math.ceil(metricReport.availableSendBandwidth / 1000)) + ' Kbps');

    } else if (typeof metricReport.availableOutgoingBitrate === 'number' && !isNaN(metricReport.availableOutgoingBitrate)) {
      (document.getElementById('video-uplink-bandwidth') as HTMLSpanElement).innerText =
        'Available Uplink Bandwidth: ' + String(Math.ceil(metricReport.availableOutgoingBitrate / 1000)) + ' Kbps';
    ...
    if (typeof metricReport.availableReceiveBandwidth === 'number' && !isNaN(metricReport.availableReceiveBandwidth)) {
      (document.getElementById('video-downlink-bandwidth') as HTMLSpanElement).innerText =
        'Available Downlink Bandwidth: ' + String(Math.ceil(metricReport.availableReceiveBandwidth / 1000)) + ' Kbps';

      // console.log('Available Downlink Bandwidth: ' + String(Math.ceil(metricReport.availableReceiveBandwidth / 1000)) + ' Kbps');


    } else if (typeof metricReport.availableIncomingBitrate === 'number' && !isNaN(metricReport.availableIncomingBitrate)) {
      (document.getElementById('video-downlink-bandwidth') as HTMLSpanElement).innerText =
        'Available Downlink Bandwidth: ' + String(Math.ceil(metricReport.availableIncomingBitrate / 1000)) + ' Kbps';

      // console.log('Available Downlink Bandwidth: ' + String(Math.ceil(metricReport.availableIncomingBitrate / 1000)) + ' Kbps');
    ...
    if (typeof metricReport.availableSendBandwidth === 'number' && !isNaN(metricReport.availableSendBandwidth)) {
      ...
      this.totalBandwidth = 'Available Total Bandwidth: ' + String(Math.ceil(metricReport.availableSendBandwidth / 1000) + Math.ceil(metricReport.availableReceiveBandwidth / 1000)) + ' Kbps'; //Math.ceil(metricReport.availableSendBandwidth / 1000) <- 소수점이 많이 나오면 사용
      (document.getElementById('video-total-bandwidth') as HTMLSpanElement).innerText = this.totalBandwidth;

    } else if (typeof metricReport.availableOutgoingBitrate === 'number' && !isNaN(metricReport.availableOutgoingBitrate)) {

      this.totalBandwidth = 'Available Total Bandwidth: ' + String(Math.ceil(metricReport.availableOutgoingBitrate / 1000) + Math.ceil(metricReport.availableIncomingBitrate / 1000)) + ' Kbps';
      (document.getElementById('video-total-bandwidth') as HTMLSpanElement).innerText = this.totalBandwidth;

    } else {
      (document.getElementById('video-total-bandwidth') as HTMLSpanElement).innerText = 'Available Total Bandwidth: Unknown';
    }

    // console.log(this.totalBandwidth);
    //   setTimeout(function(totalBandwidth : any) {
    //     console.log(totalBandwidth);
    //   },10000,this.totalBandwidth);
  ...
  setupDeviceLabelTrigger(): void {
    // 장치 레이블은
    // 브라우저 세션의 핑거 프린팅 표면 영역입니다. Chrome 비공개
    // 탭과 모든 Firefox 탭에서 레이블은 한 번만 읽을 수 있습니다.
    // MediaStream이 활성화되었습니다. 이 제한을 처리하는 방법은
    // 원하는 UX. 장치 컨트롤러에는 주사 가능한 장치 레이블이 포함되어 있습니다.
    //없는 경우 사용자 지정 동작을 수행 할 수있는 트리거
    // 잠금을 해제하기위한 임시 오디오 / 비디오 스트림 생성과 같은 라벨
    // 기본 동작 인 장치 이름. 여기서 우리는
    // 사용자에게 요청하고 있음을 알리는 경고도 표시하도록 트리거합니다.
    // 마이크 / 카메라 권한.
    //
    // 또한 Firefox에는 자체 장치 선택기가있어 유용 할 수 있습니다.
    // 첫 번째 장치 선택을 위해. 후속 장치 선택은 다음을 사용할 수 있습니다.
    // 특정 장치 ID가있는 사용자 지정 UX입니다.
